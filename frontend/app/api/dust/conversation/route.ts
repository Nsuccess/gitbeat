...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeAgents.length === 0) {
      throw new Error("No active agents found in your Dust.tt workspace. Please create an assistant at https://dust.tt");
    }

    // Prefer "deep-dive" agent for comprehensive repository analysis, fallback to first active agent
    const deepDiveAgent = activeAgents.find(agent => agent.sId === "deep-dive");
    const selectedAgent = deepDiveAgent || activeAgents[0];
    ...
    const conversationResult = await dustAPI.createConversation({
      ...
      message: {
        ...
        mentions: mentions || [
          {
            configurationId: selectedAgent.sId, // Use the selected agent ID
          },
        ],
        context: {
